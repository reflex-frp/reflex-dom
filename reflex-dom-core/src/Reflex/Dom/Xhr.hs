...
import Language.Javascript.JSaddle.Monad (JSM, MonadJSM, askJSM, liftJSM, runJSM)
...
instance a ~ () => Default (XhrRequestConfig a) where
  def =
    XhrRequestConfig
      { _xhrRequestConfig_headers = Map.empty,
...
parseAllHeadersString s =
  Map.fromList $
    fmap (stripBoth . T.span (/= ':')) $
      L.dropWhileEnd T.null $
        T.splitOn (T.pack "\r\n") s
  where
    stripBoth (txt1, txt2) = (CI.mk $ T.strip txt1, T.strip $ T.drop 1 txt2)
